...
template <typename T> class IndexPackedPolyVec {
...
public:
    ...
    template <typename U, typename ...Args> void emplace_back(Args&&... args) {
        ...
        *reinterpret_cast<SizeEntry_t*>(&data[size_]) = EntrySize<U>();
        ...
        indexToOffset.emplace_back(&data[size_ + sizeof(SizeEntry_t)]);
        size_ += EntrySize<U>();
    }
...
void fill(IndexPackedPolyVec<Base>& vec, const int N) {
...
int sum(IndexPackedPolyVec<Base>& vec) {
    int sum = 0;
    for (auto& b : vec) {
        sum += b.getData();
    }
